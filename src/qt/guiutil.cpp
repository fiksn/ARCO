...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("aquariuscoin"))
        return false;
...
{
    // Convert aquariuscoin:// to aquariuscoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("aquariuscoin://"))
    {
        uri.replace(0, 12, "aquariuscoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "AquariusCoin.lnk";
}
...
{
    return GetAutostartDir() / "aquariuscoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("AquariusCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  aquariuscoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("AquariusCoin-Qt"));
